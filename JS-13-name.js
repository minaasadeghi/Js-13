/* 
Write a code that every letter of the alphabet that the user enters
...
function getNamesByLetter() {
  ...
  if (!input || input.length !== 1 || !isNaN(input)) {
    alert("Please enter only one alphabet letter.");
    return;
  }

  const filteredNames = names.filter(name => name[0].toLowerCase() === input);
